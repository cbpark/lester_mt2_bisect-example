...
precision.
                                  // If >0, MT2 computed to supplied absolute
precision.
...
on one parameter Fernando Etayo 1,3, Laureano ,2,3, Natalia del Rio 3Vega
Departamento de Matematicas, Estadistica y Computacion, Universidad de
...
of Computer Aided Geometric Design 23 (2006) 350, the authors have indicated
...
be the subject of a paper (to appear) from Etayo and Gonzalez-Vega.324
 *
 *
 ...
 * with (non-singular) ellipses are fed into the program below.  For our
...
struct EllipseParams {
    ...
    inline
#endif
        EllipseParams(const double c_xx2, const double c_yy2,
                      const double c_xy2, const double c_x2, const double c_y2,
                      const double c2)
        : c_xx(c_xx2), c_yy(c_yy2), c_xy(c_xy2), c_x(c_x2), c_y(c_y2), c(c2) {
        // Etayo et al REQUIRE that c_xx and c_yy are non-negative, so:
        if (c_xx < 0 || c_yy < 0) { throw "precondition violation"; }
        setDet();
    }
    EllipseParams() {}
#ifdef ENABLE_INLINING
...
#endif
        bool
        operator==(const EllipseParams &other) const {
...
bool ellipsesAreDisjoint(const EllipseParams &e1, const EllipseParams &e2) {
    /* We want to construct the polynomial "Det(lambda A + B)" where A and B are
    the 3x3 matrices associated with e1 and e2, and we want to get that
    polynomial in the form lambda^3 + a lambda^2 + b lambda + c.


    Note that by default we will not have unity as the coefficient of the
    lambda^3 term, however the redundancy in the parametrisation of A and B
    allows us to scale the whole ply until the first term does have a unit
    coefficient.
    */

    if (e1 == e2) {
        return false;  // Probably won't catch many cases, but may as well have
                       // it here.
    }
    ...
    // Since question is "symmetric" and since we need to divide by coeffLamPow3
...
public:
    ...
    get_mT2(  // returns asymmetric mT2 (which is >=0), or returns a negative
        ...
        const double desiredPrecisionOnMT2 =
            0,  // This must be non-negative.  If set to zero (default) MT2 will
                // be calculated to the highest precision available on the
                // machine (or as close to that as the algorithm permits).  If
                // set to a positive value, MT2 (note that is MT2, not its
                // square) will be calculated to within +-
                // desiredPrecisionOnMT2. Note that by requesting precision of
                // +- 0.01 GeV on an MT2 value of 100 GeV can result in speedups
                // of a factor of two or three.
        const bool useDeciSectionsInitially =
            true  // If true, interval is cut at the 10% point until first
                  // acceptance, which gives factor 3 increase in speed
                  // calculating kinematic min, but 3% slowdown for events in
                  // the bulk.  Is on (true) by default, but can be turned off
                  // by setting to false.
    ) {
        ...
        if (mT2_Sq == MT2_ERROR) { return MT2_ERROR; }
        return sqrt(mT2_Sq);
    }
    ...
    static void disableCopyrightMessage(const bool printIfFirst = false) {
        ...
        if (first && printIfFirst) {
            std::cout
                ...
                << "#     asymm_mt2_lester_bisect::disableCopyrightMessage();\n"
                << "# \n"
                << "# somewhere before you begin to calculate your MT2 "
                   "values.\n"
                << "#========================================================="
                   "\n"
    ...
    static double get_mT2_Sq(  // returns square of asymmetric mT2 (which is
                               // >=0), or returns a negative number (such as
        ...
        const double desiredPrecisionOnMT2 =
            0,  // This must be non-negative.  If set to zero (default) MT2 will
                ...
                // square) will be calculated to within +-
                // desiredPrecisionOnMT2. Note that by requesting precision of
                // +- 0.01 GeV on an MT2 value of 100 GeV can resJult in
                // speedups of a factor of ..
        const bool useDeciSectionsInitially =
            true  // If true, interval is cut at the 10% point until first
                  // acceptance, which gives factor 3 increase in speed
                  // calculating kinematic min, but 3% slowdown for events in
        ...
        disableCopyrightMessage(
            true);  // By supplying an argument to disable, we actually ask for
                    // the message to be printed, if printing is not already
                    // disabled.   This counterintuitive function naming is to
                    // avoid the need to introduce static variable
                    // initialisations ....
#endif

        const double m1Min =
            mVis1 + mInvis1;  // when parent has this mass, ellipse 1 has
                              // smallest physical size
        const double m2Min =
            mVis2 + mInvis2;  // when parent has this mass, ellipse 2 has
                              // smallest physical size
        ...
        if (m1Min > m2Min) {
            ...
            return asymm_mt2_lester_bisect::get_mT2_Sq(
                ...
                mInvis2, mInvis1, desiredPrecisionOnMT2);
        }
        ...
        const double mMin =
            m2Min;  // when parent has this mass, both ellipses are physical,
                    // and at least one has zero size.  Note that the name "min"
                    // expresses that it is the minimum potential parent mass we
                    // should consider, not that it is the min of m1Min and
                    // m2Min.  It is in fact the MAX of them!

        // TODO: What about rounding?  What about idiots who give us mVis values
        // that have been computed from E^2-p^2 terms that are perilously close
        // to zero, or perilously degenerate?
        ...
        std::cout << "\nMOO ";
#endif
        // Check for an easy MT2 zero, not because we think it will speed up
        // many cases, but because it will allow us to, ever after, assume that
        // scaleSq>0.
        ...
        double mUpper =
            mMin +
            scale;  // since scaleSq is guaranteed to be >0 at this stage, the
        ...
        while (desiredPrecisionOnMT2 <= 0 ||
            ...
            const double trialM = (goLow ? (mLower * 15 + mUpper) /
                                               16  // bias low until evidence
                                                   // this is not a special case
                                         : (mUpper + mLower) / 2.0  // bisect
            );  // worry about this not being between mUpperSq and mLowerSq!
                // TODO

            if (trialM <= mLower || trialM >= mUpper) {
                // We reached a numerical precision limit:  the interval can no
                // longer be bisected!
#ifdef LESTER_DBG
            ...
            try {
                ...
                // The test for ellipses being disjoint failed ... this means
                // the ellipses became degenerate, which can only happen right
                // at the bottom of the MT2 search range (subject to numerical
                // precision).  So:
#ifdef LESTER_DBG
...
#endif
        static double
        lestermax(const double x, const double y) {
        return (x > y) ? x : y;
    }
    static const Lester::EllipseParams helper(
        const double mSq,  // The test parent-mass value (squared)
...
void myversion() { std::cout << "Version is : 2014_11_13" << std::endl; }
...
std::pair<double, double> ben_findsols(double MT2, double px, double py,
    ...
    if (TermSqy1 * TermSqy1 - 4. * TermSqy0 * TermSqy2 < 0 ||
        ...
        for (double metpy = low; metpy <= high;
             metpy += (high - low) / 10000.) {
            double metpx = -(TermB * metpy + TermA -
                             sqrt(TermSqy0 + TermSqy1 * metpy +
                                  TermSqy2 * metpy * metpy)) *
                           0.5 / (E2 - px2);
            double metpx2 = -(TermB * metpy + TermA +
                              sqrt(TermSqy0 + TermSqy1 * metpy +
                                   TermSqy2 * metpy * metpy)) *
                            0.5 / (E2 - px2);
